...
const isActive = "!bg-accent-400/40";
...
const MenuBar = ({ editor, className }: MenuBarProps) => {
  ...
  const editorStructureButtons: EditorButton[] = [
    {
      type: "button",
  ...
  const editorRedoButtons: EditorButton[] = [
    {
      type: "button",
  ...
  return (
    <ToggleButtonGroup
      className={cn(
        "border-primary-600/30 bg-primary-950 flex flex-wrap !rounded-b-none border p-2 shadow-md",
        className,
      )}
    >
      ...
      {editorStructureButtons.map((button, index) => (
        <ToggleButton
          ...
          onClick={button.onClick}
          disabled={button.disabled ? button.disabled : false}
          className={cn("!m-0.5 !rounded-md !border-0 !p-2", button.className)}
          sx={{
            px: 1,
          }}
        >
